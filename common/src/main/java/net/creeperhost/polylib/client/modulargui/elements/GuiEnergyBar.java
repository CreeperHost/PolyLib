...
public class GuiEnergyBar extends GuiElement<GuiEnergyBar> implements BackgroundRender {
    ...
    public static BiFunction<Long, Long, List<Component>> defaultFormatter() {
        return (energy, capacity) -> {
            ...
            tooltip.add(new TranslatableComponent("energy_bar.polylib.energy_storage").withStyle(DARK_AQUA));
            boolean shift = Screen.hasShiftDown();
            tooltip.add(new TranslatableComponent("energy_bar.polylib.capacity")
                    .withStyle(GOLD)
                    .append(" ")
                    .append(new TextComponent(shift ? FormatHelper.addCommas(capacity) : FormatHelper.formatNumber(capacity))
                            .withStyle(GRAY)
                            .append(" ")
                            .append(new TranslatableComponent("energy_bar.polylib.rf")
                                    .withStyle(GRAY)
                            )
                    )
            );
            tooltip.add(new TranslatableComponent("energy_bar.polylib.stored")
                    .withStyle(GOLD)
                    .append(" ")
                    .append(new TextComponent(shift ? FormatHelper.addCommas(energy) : FormatHelper.formatNumber(energy))
                            .withStyle(GRAY)
                    )
                    .append(" ")
                    .append(new TranslatableComponent("energy_bar.polylib.rf")
                    ...
                    .append(new TextComponent(String.format(" (%.2f%%)", ((double) energy / (double) capacity) * 100D))
                            .withStyle(GRAY)
                    )
            );
            return tooltip;
