...
public class GuiFluidTank extends GuiElement<GuiFluidTank> implements BackgroundRender {
    ...
    public static BiFunction<FluidStack, Long, List<Component>> defaultFormatter() {
        return (fluidStack, capacity) -> {
            ...
            tooltip.add(new TranslatableComponent("fluid_tank.polylib.fluid_storage").withStyle(DARK_AQUA));
            if (!fluidStack.isEmpty()) {
                tooltip.add(new TranslatableComponent("fluid_tank.polylib.contains")
                        .withStyle(GOLD)
                        .append(" ")
            ...
            tooltip.add(new TranslatableComponent("fluid_tank.polylib.capacity")
                    .withStyle(GOLD)
                    .append(" ")
                    .append(new TextComponent(FormatHelper.addCommas(capacity))
                            .withStyle(GRAY)
                            .append(" ")
                            .append(new TranslatableComponent("fluid_tank.polylib.mb")
                                    .withStyle(GRAY)
                            )
                    )
            );
            tooltip.add(new TranslatableComponent("fluid_tank.polylib.stored")
                    .withStyle(GOLD)
                    .append(" ")
                    .append(new TextComponent(FormatHelper.addCommas(fluidStack.getAmount()))
                            .withStyle(GRAY)
                    )
                    .append(" ")
                    .append(new TranslatableComponent("fluid_tank.polylib.mb")
                    ...
                    .append(new TextComponent(String.format(" (%.2f%%)", ((double) fluidStack.getAmount() / (double) capacity) * 100D))
                            .withStyle(GRAY)
                    )
            );
            return tooltip;
