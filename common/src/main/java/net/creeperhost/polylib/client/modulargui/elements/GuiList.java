...
public class GuiList<E> extends GuiElement<GuiList<E>> {
    ...
    private BiFunction<GuiList<E>, E, ? extends GuiElement<?>> displayBuilder = (parent, e) -> {
        GuiText text = new GuiText(parent, () -> new TextComponent(String.valueOf(e))).setWrap(true);
        text.constrain(GeoParam.HEIGHT, Constraint.dynamic(() -> (double) font().wordWrapHeight(text.getText() == null ? "" : text.getText().getString(), (int) text.xSize())));
        return text;
    };
    ...
    public SliderState scrollState() {
        return SliderState.forScrollBar(() -> yScrollPos, e -> {
            yScrollPos = e;
            updateVisible();
        }, () -> MathUtil.clamp(ySize() / contentHeight, 0, 1));
    }
