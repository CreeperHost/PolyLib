...
public class ScreenWell
{
    ...
    public void render(GuiGraphicsExtractor guiGraphics)
    {
        ...
        guiGraphics.text(fontRenderer, title, this.left + ((this.right - this.left) / 2) - (titleWidth / 2), this.top + 2, 0xFFFFFF, true);
        ...
        for (String line : lines)
        {
            if (centeredF)
            {
                ...
                guiGraphics.text(fontRenderer, I18n.get(line), this.left + ((this.right - this.left) / 2) - (stringWidth / 2), topStart, 0xFFFFFF, true);
            } else
            {
                guiGraphics.text(fontRenderer, I18n.get(line), this.left, topStart, 0xFFFFFF, true);
            }
            topStart += 10;
        }
    }
