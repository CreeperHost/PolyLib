...
public class SimpleToast implements Toast {
    private static final Identifier BG_TEXTURE = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "textures/toast.png");
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, Font font, long l) {
        ...
        if (title != null) {
            ...
            int n = 0xFF88FF;
            if (list.isEmpty()) {
                guiGraphics.textWithWordWrap(Minecraft.getInstance().font, title, xOffset, 7, wrapWidth, n | 0xFF000000);
            } else if (list.size() == 1) {
                guiGraphics.text(Minecraft.getInstance().font, title, xOffset, 7, n | 0xFF000000);
                guiGraphics.text(Minecraft.getInstance().font, list.getFirst(), xOffset, 18, -1);
            } else {
                if (l < 1500L) {
                    int k = Mth.floor(Mth.clamp((float) (1500L - l) / 300.0f, 0.0f, 1.0f) * 255.0f) << 24 | 0x4000000;
                    guiGraphics.text(Minecraft.getInstance().font, title, xOffset, 11, n | k);
                } else {
                    ...
                    for (FormattedCharSequence formattedCharSequence : list) {
                        guiGraphics.text(Minecraft.getInstance().font, formattedCharSequence, xOffset, m, 0xFFFFFF | k);
                        m += font.lineHeight;
                    }
                }
            }
            visibility = l >= 5000L ? Visibility.HIDE : Visibility.SHOW;
        } else {
