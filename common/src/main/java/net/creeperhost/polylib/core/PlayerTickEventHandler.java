            ...
            if (player != null)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (!stack.isEmpty())
                    {
                        ...
                        {
                            ArmourChangedEvent.ARMOUR_EQUIPPED_EVENT.invoker().onArmourEquipped(player, current[i],
                                    stack.copy());
                        }
                    ...
                    {
                        //Armour has changed
                        ArmourChangedEvent.ARMOUR_UNEQUIPPED_EVENT.invoker().onArmourUnEquipped(player, current[i],
                                stack.copy());
                        current[i] = stack.copy();
                    }
                }
