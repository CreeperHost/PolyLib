...
public class PolyFluidStorage implements IPolyFluidStorage, Serializable {
    ...
    public PolyFluidStorage setCapacity(long capacity) {
        ...
        if (fluid.getAmount() > this.capacity) {
            setFluid(fluid.copyWithAmount(this.capacity));
        }
        return this;
    }
    ...
    public long fill(PolyFluidStack resource, boolean simulate) {
        ...
        long filled = Math.min(Math.max(0, capacity - fluid.getAmount()), resource.getAmount());
    ...
    public void deserialize(ValueInput input) {
        PolyFluidStack loaded = input.read("fluid", PolyFluidStack.CODEC).orElse(PolyFluidStack.EMPTY);
        if (loaded.getFluid() == Fluids.EMPTY) {
            fluid = PolyFluidStack.EMPTY;
        } else {
            fluid = loaded.copyWithAmount(Math.min(loaded.getAmount(), capacity));
        }
    }
