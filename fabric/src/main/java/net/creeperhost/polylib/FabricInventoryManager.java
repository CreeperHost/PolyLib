...
public class FabricInventoryManager
{
    ...
    public static final FluidManager FLUID_MANAGER = new FabricFluidManager();
    ...
    public static void init()
    {
        ...
        FluidStorage.ITEM.registerFallback((itemStack, context) -> {
            if (itemStack.getItem() instanceof PolyFluidItem fluidItem) {
                IPolyFluidStorageItem storage = fluidItem.getFluidStorage(itemStack);
                return storage == null ? null : new PolyFabricFluidItemWrapper(storage, context);
            }
            return null;
        });
