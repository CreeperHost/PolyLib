...
public class PolyNeoFluidWrapper implements ResourceHandler<FluidResource> {
    ...
    public long getCapacityAsLong(int index, FluidResource resource) {
        return index == 0 && (resource.isEmpty() || storage.isFluidValid(NeoPolyFluidWrapper.fromNeo(resource, 1)))
    ...
    public boolean isValid(int index, FluidResource resource) {
        return index == 0 && storage.isFluidValid(NeoPolyFluidWrapper.fromNeo(resource, 1));
    }

    @Override
    public int insert(int index, FluidResource resource, int amount, TransactionContext transaction) {
        if (index != 0 || resource.isEmpty() || amount <= 0) return 0;
        PolyFluidStack stack = NeoPolyFluidWrapper.fromNeo(resource, amount);
